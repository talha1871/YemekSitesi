...
namespace YemekSitesiUI.Controllers
{
    public class ContactController : Controller
    {
        ...
        public async Task<IActionResult> CreateContact(CreateContactDto createContactDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7254/api/Contact", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateContact(UpdateContactDto updateContactDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:7254/api/Contact/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
